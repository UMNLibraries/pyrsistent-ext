...
class _PRecordExtMeta(type):
    def __new__(mcs, name, bases, dct):
        ...
        return super(_PRecordExtMeta, mcs).__new__(mcs, name, bases, dct)


class PRecordExt(PMap, CheckedType, metaclass=_PRecordExtMeta):
    """
    A PRecordExt is a PMap with a fixed set of specified fields. Records are declared as python classes inheriting
    from PRecordExt. Because it is a PMap it has full support for all Mapping methods such as iteration and element
    access using subscript notation.

    More documentation and examples of PRecordExt usage is available at https://github.com/tobgu/pyrsistent
    """
    def __new__(cls, **kwargs):
        ...
        if '_precord_size' in kwargs and '_precord_buckets' in kwargs:
            return super(PRecordExt, cls).__new__(cls, kwargs['_precord_size'], kwargs['_precord_buckets'])
        ...
        #e = _PRecordExtEvolver(cls, pmap(pre_size=len(cls._precord_fields)), _factory_fields=factory_fields, _ignore_extra=ignore_extra)
        e = _PRecordExtEvolver(cls, pmap(pre_size=len(cls._precord_fields)), _factory_fields=factory_fields)
    ...
    def set(self, *args, **kwargs):
        ...
        # The PRecordExt set() can accept kwargs since all fields that have been declared are
        # valid python identifiers. Also allow multiple fields to be set in one operation.
        if args:
            return super(PRecordExt, self).set(args[0], args[1])
    ...
    def evolver(self):
        ...
        """
        return _PRecordExtEvolver(self.__class__, self)
    ...
    #def create(cls, kwargs, _factory_fields=None):
        """
        Factory method. Will create a new PRecordExt of the current type and assign the values
        specified in kwargs.

        #:param ignore_extra: A boolean which when set to True will ignore any keys which appear in kwargs that are not
        #                     in the set of fields on the PRecordExt.
        """
    ...
    def serialize(self, format=None):
        """
        Serialize the current PRecordExt using custom serializer functions for fields where
        such have been supplied.
        """
        return dict((k, serialize(self._precord_fields[k].serializer, format, v)) for k, v in self.items())


class _PRecordExtEvolver(PMap._Evolver):
    ...
    def __init__(self, cls, original_pmap, _factory_fields=None):
        super(_PRecordExtEvolver, self).__init__(original_pmap)
    ...
    def set(self, key, original_value):
        ...
        if field:
            if self._factory_fields is None or field in self._factory_fields:
                try:
#                    if is_field_ignore_extra_complaint(PRecordExt, field, self._ignore_extra):
...
#                        value = field.factory(original_value)
        ...
        return super(_PRecordExtEvolver, self).set(key, value)

    def persistent(self):
        cls = self._destination_cls
        is_dirty = self.is_dirty()
        pm = super(_PRecordExtEvolver, self).persistent()
